...
from wwise_helpers import show_error_message, set_client, get_selected_items, show_message
...
def get_loudness_for_wavs(client, objects):
    for i in objects:
        ...
        i["loudness"] = loudness
        notes = f'{i["name"]} loudness = {i["loudness"]} LUFS'
        # show_message("Message", notes)
    return objects
...
def calculate_average_loudness(client, items):
    ...
    for parent_id, data in parent_loudness.items():
        ...
        avg = f'Average loudness for {parent_name} = {round(average_loudness[parent_id], 2)} LUFS'
        show_message("Loudness", avg)
