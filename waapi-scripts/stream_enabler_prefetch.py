...
def check_for_stream(client, child_id, child_name, user_input):
    ...
    if is_stream:
        print(f"Streaming is already enabled for {child_name}")
        return
    else:
        enable_stream(client, child_id, child_name, user_input)


def enable_stream(client, child_id, child_name, user_input):
    args = {
        "objects": [{
            "object": child_id,
    ...
    print(f"Streaming is now enabled for {child_name}")
        ...
        with client:
            ...
            for item in selected_items:
                ...
                for child in children:
                    ...
                    check_for_stream(client, child_id, child_name, user_input)
                    changed_items += 1

        show_success_message(f"Stream enabled for {changed_items} selected item(s)")
        client.disconnect()
